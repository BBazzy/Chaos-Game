...
Feriga <- function(distanceRatio)
{
  ...
  RandomPointY <- runif(1, 0, 1)
  Points[1, ] <- c(RandomPointX, RandomPointY)
  PreviousRandomVertex=0
  for(i in 1:(NMAX - 1))
  {  
    
    RandomProb <- runif(1,0,1)
    
    if(RandomProb<0.01)
    {
      NextPointX <-0;
      NextPointY <-0.16*RandomPointY
      Points[i + 1, ] <- c(NextPointX, NextPointY)
      RandomPointX <- NextPointX
      RandomPointY <- NextPointY
    }
    else if(RandomProb<0.86)
    {
      NextPointX <-0.85*RandomPointX+0.04*RandomPointY;
      NextPointY <--0.04*RandomPointX+0.85*RandomPointY+1.6;
      Points[i + 1, ] <- c(NextPointX, NextPointY)
      RandomPointX <- NextPointX
      RandomPointY <- NextPointY
    }
    else if(RandomProb<0.93)
    {
      NextPointX <-0.20*RandomPointX - 0.26*RandomPointY;
      NextPointY <-0.23*RandomPointX+  0.22*RandomPointY+1.6;
      Points[i + 1, ] <- c(NextPointX, NextPointY)
      RandomPointX <- NextPointX
      RandomPointY <- NextPointY
    }
    ...
    {
      NextPointX <- -0.15*RandomPointX + 0.28*RandomPointY;
      NextPointY <-0.26*RandomPointX+  0.24*RandomPointY+0.44;
      Points[i + 1, ] <- c(NextPointX, NextPointY)
      RandomPointX <- NextPointX
      RandomPointY <- NextPointY
    }
...
server <- function(input, output)
{
  figure <-reactive({
    ...
    if(input$shape == "feriga")
    {
      return(Feriga(input$dist.feriga))
    }
